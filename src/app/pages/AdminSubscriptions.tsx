...
import { getAdminSubscriptionPlans, updateAdminSubscriptionPlan } from '@/api/admin'
...
export function AdminSubscriptions() {
  ...
  const totalRevenueProjection = useMemo(
    () => plans.reduce((sum, p) => sum + p.price * p.durationDays, 0),
    [plans],
  )
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-6 text-white">
        ...
        <p className="mt-1 text-sm text-slate-200">
          Quản lý bảng giá, thời hạn và mô tả các gói đang bán cho hướng dẫn viên.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardContent className="p-4">
            <p className="text-xs uppercase tracking-wide text-gray-500">Chỉ số giá x ngày</p>
            <p className="mt-1 text-2xl font-bold">{formatVND(totalRevenueProjection)}</p>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {plans.map((plan) => (
                <div key={plan.plan} className="rounded-xl border p-4 transition hover:shadow-sm">
                  ...
                  <p className="mb-3 text-2xl font-bold text-emerald-600">{formatVND(plan.price)}</p>
      ...
      <Dialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="capitalize">Cập nhật gói {selected?.plan}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="rounded-lg border bg-slate-50 p-3 text-xs text-slate-600">
              <p className="font-medium text-slate-800">Gợi ý</p>
              <p className="mt-1">
                Chọn giá theo chiến lược thị trường. Mô tả nên nêu rõ quyền lợi để tăng tỉ lệ chuyển đổi.
              </p>
            </div>
          </div>
          <DialogFooter>
