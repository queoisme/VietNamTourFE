...
import { uploadPostImages } from '@/api/uploads'
...
export function PostImageUpload({ urls, onChange, maxImages = 10, disabled }: Props) {
  ...
  async function handleFiles(files: FileList | null) {
    ...
    setUploading(true)
    try {
      const newUrls = await uploadPostImages(picked)
    ...
    <div className="space-y-2">
      {urls.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {urls.map((url) => (
            <div key={url} className="group relative size-24 overflow-hidden rounded-xl border bg-slate-100">
              <img src={url} alt="post" className="size-full object-cover" />
              {!disabled && (
                <button
                  type="button"
                  onClick={() => remove(url)}
          ...
          {uploading && (
            <div className="flex size-24 items-center justify-center rounded-xl border border-dashed bg-slate-50">
        ...
        <button
          ...
          className={cn(
            ...
            uploading
              ...
              : 'border-slate-300 text-slate-500 hover:border-orange-400 hover:bg-orange-50 hover:text-orange-600',
          )}
        >
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-3">
        {images.map((url, i) => (
          <button
            ...
            className="aspect-square overflow-hidden rounded-xl border bg-slate-100 transition-opacity hover:opacity-90"
          >
